...
@router.get("/")
async def get_all_teachers(session: SessionDep, current_user: UserDep) -> list[Teacher]:
    """Return all teachers."""
    return session.exec(select(Teacher)).all()


@router.post("/")
async def add_teacher(teacher: Teacher, session: SessionDep, current_user: UserDep) -> Teacher:
...
async def get_teacher(teacher_id: int, session: SessionDep, current_user: UserDep) -> Teacher:
...
async def delete_teacher(teacher_id: int, session: SessionDep, current_user: UserDep) -> dict:
    """Delete a teacher by ID."""
    if current_user.role not in [Roles.ADMIN, Roles.SECRETARY]:
        raise HTTPException(status_code=403, detail="Not authorized to delete a teacher")
    teacher = session.get(Teacher, teacher_id)
    if not teacher:
        raise HTTPException(status_code=404, detail="Teacher not found")
...
async def update_teacher(teacher_id: int, teacher: Teacher, session: SessionDep, current_user: UserDep) -> Teacher:
    """Update a teacher by ID and return it as a response."""
    if current_user.role == Roles.STUDENT:
        raise HTTPException(status_code=403, detail="Not authorized to update a teacher")
    
    db_teacher = session.get(Teacher, teacher_id)
    if not db_teacher:
        raise HTTPException(status_code=404, detail="Teacher not found")
    if current_user.id != db_teacher.user_id and current_user.role == Roles.TEACHER:
        raise HTTPException(status_code=403, detail="You can only update your own teacher profile")
